           ...
           <a href="{{ url('/admin/home ') }}" class="active">Home</a>
            ...
            <a href="#pageSubmenu" data-toggle="collapse" aria-expanded="false">Manage Users</a>
                <ul class="collapse list-unstyled" id="pageSubmenu">
                    <li class="active"><a href="">Green Captain</a></li>
             ...
             <tbody>
                 @foreach($gcs as $gc)
                    <tr>
                            ...
                            <td> {{$gc->name}} </td>  
                            <td> {{$gc->email}} </td>
                            <td><p data-placement="top" data-toggle="tooltip" title="Edit"><button class="btn btn-primary btn-xs" data-title="Edit" data-toggle="modal" data-target="#{{$gc->id}}" ><span class="glyphicon glyphicon-pencil"></span></button></p></td>
                            <td><p data-placement="top" data-toggle="tooltip" title="Delete"><button class="btn btn-danger btn-xs" data-title="Delete" data-toggle="modal" data-target="#delete" ><a href="/admin/deletegc/{{$gc->id}}"><span class="glyphicon glyphicon-trash"></span></button></p></td>
                    </tr>
              
                    <form method="POST" action="updategc/{{$gc->id}}">
                    @csrf
                    <!-- View Profile Button Popup -->
                            <div class="modal fade" id="{{$gc->id}}" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
                                ...
                                <div class="modal-content">
                                  ...
                                  <!--  <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button -->
                                    ...
                                    <input class="form-control " type="text" name="id" value="{{$gc->id}}">
                                    </div>

                                    <label class="col-md-4">Name</label>
                                    <div class="form-group">
                                    <input class="form-control " type="text" name="name" value="{{$gc->name}}" >
                                    </div>

                                    <label class="col-md-4">E-mail</label>
                                    <div class="form-group">
                                    <input class="form-control " type="text" name="email" value="{{$gc->email}}" >
                                    </div>
                                    
                                    <label class="col-md-4">Mobile</label>
                                    <div class="form-group">
                                    <input class="form-control " type="text" name="number" value="{{$gc->number}}" >
                                    </div>
